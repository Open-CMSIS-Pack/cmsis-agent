...
- **Expected input:** Existing target `.pdsc`, selected family/subFamily/device/variant scope, affected processors and non-trace sequences, documented device behavior, and a current `.agent-artifacts/<pdsc-stem>.debug-knowledge.md` record.
- **Dependencies:** `$debug-knowledge`; `$prepare-pdsc-sequence-change`; `$manage-pdsc-debugvars`; `$apply-confirmed-pdsc-proposal`; `$validate-pdsc-sequence-xml`; current [Open-CMSIS-Pack *Debug Description* specification](https://open-cmsis-pack.github.io/Open-CMSIS-Pack-Spec/main/html/debug_description.html); and `references/debug-sequence-contract.md`.
...
1. **Analysis:** Read the debug knowledge record and run `$prepare-pdsc-sequence-change` with `domain=debug`, passing verified facts in conversation. Require current connection facts and documented low-power behavior; return insufficient facts to `$debug-knowledge`.
2. **Processing:** Read `references/debug-sequence-contract.md` and assemble only verified non-trace sequences with correct CPU applicability, placement, predefined-default provenance, and low-power behavior. For every documented user runtime choice, run `$manage-pdsc-debugvars` with prepared placement context and merge its returned definition and sequence use into this skill's private record.
