...
Use CoreSight documentation to establish only component-local lifecycle programming: initialization, enable/capture, flush, stop, and status polling for the identified component and version. Cite the component/version and exact section. Do not use it to infer SoC or board integration, component address, clock or reset control, pin configuration, or routing; require device-level evidence for those facts. Board evidence may supplement an optional configuration but cannot limit device-level Pack options. Before recording a sink-lifecycle blocker, complete this CoreSight evidence pass.
